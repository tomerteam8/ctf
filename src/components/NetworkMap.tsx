...
// ── Layout helpers ────────────────────────────────────────────────────────────
// Device positions inside zones (relative to zone top-left)
// 2×2 grid: zone 295×270
const G22 = [
  { x: 12, y: 42 }, { x: 152, y: 42 },
  { x: 12, y: 152 }, { x: 152, y: 152 },
];
// 3+2 grid: zone 420×270 (3 per row, then 2 centered)
const G32 = [
  { x: 10, y: 42 }, { x: 148, y: 42 }, { x: 286, y: 42 },
  { x: 79, y: 152 }, { x: 217, y: 152 },
];
// 1×3 row: zone 420×160
const G13 = [
  { x: 10, y: 42 }, { x: 148, y: 42 }, { x: 286, y: 42 },
];
...
const NODES: Node[] = [
  ...
  // ── Cloudflare (standalone device node) ───────────────────────────────────
  {
    id: 'cloudflare', type: 'device',
    position: { x: 840, y: 50 },
    data: { label: 'Cloudflare WAF', ip: '1.1.1.1 / CDN', iconType: 'waf', color: '#f97316', bg: 'rgba(249,115,22,0.08)' },
  ...
  devNode('ep-admin',   'zone-endpoints', G32[0], { label: 'Admin',     ip: '192.168.1.10', port: 'VPN', iconType: 'ep-admin',   color: '#ef4444', bg: 'rgba(239,68,68,0.08)',   services: 'Admin workstation'   }),
  devNode('ep-it',      'zone-endpoints', G32[1], { label: 'IT',        ip: '192.168.1.11', port: 'VPN', iconType: 'ep-it',      color: '#3b82f6', bg: 'rgba(59,130,246,0.08)',  services: 'IT workstation'      }),
  devNode('ep-manager', 'zone-endpoints', G32[2], { label: 'Manager',   ip: '192.168.1.12', port: 'VPN', iconType: 'ep-manager', color: '#3b82f6', bg: 'rgba(59,130,246,0.08)',  services: 'Manager workstation' }),
  devNode('ep-finance', 'zone-endpoints', G32[3], { label: 'Finance',   ip: '192.168.1.13', port: 'VPN', iconType: 'ep-finance', color: '#3b82f6', bg: 'rgba(59,130,246,0.08)',  services: 'Finance workstation' }),
  devNode('ep-dev',     'zone-endpoints', G32[4], { label: 'Developer', ip: '192.168.1.14', port: 'VPN', iconType: 'ep-dev',     color: '#00ff88', bg: 'rgba(0,255,136,0.07)',   services: 'Dev workstation'     }),
  ...
  devNode('nginx',           'zone-web-dmz', G22[0], { label: 'Web Server',      ip: '203.0.113.10', port: '80/443', iconType: 'webserver', color: '#00f0ff', bg: 'rgba(0,240,255,0.06)',   isTarget: true, services: 'nginx 1.24' }),
  devNode('graphql-api',     'zone-web-dmz', G22[1], { label: 'GraphQL API',     ip: '203.0.113.10', port: '/graphql', iconType: 'graphql', color: '#e040fb', bg: 'rgba(224,64,251,0.07)', isTarget: true, services: 'introspect on' }),
  devNode('search-api',      'zone-web-dmz', G22[2], { label: 'Search API',      ip: '203.0.113.10', port: '/search', iconType: 'search',  color: '#00f0ff', bg: 'rgba(0,240,255,0.06)',   isTarget: true, services: 'SQLi vuln' }),
  devNode('payment-webhook', 'zone-web-dmz', G22[3], { label: 'Pay Webhook',     ip: '203.0.113.10', port: '/webhooks', iconType: 'session', color: '#00ff88', bg: 'rgba(0,255,136,0.06)', isTarget: true, services: 'Stripe replay' }),
  ...
  devNode('ftp',        'zone-net-dmz', G13[0], { label: 'FTP Server',   ip: '203.0.113.11', port: '21',   iconType: 'ftp',   color: '#f59e0b', bg: 'rgba(245,158,11,0.06)', isTarget: true, services: 'vsftpd 3.0' }),
  devNode('smtp',       'zone-net-dmz', G13[1], { label: 'SMTP Server',  ip: '203.0.113.12', port: '25',   iconType: 'smtp',  color: '#f59e0b', bg: 'rgba(245,158,11,0.06)', isTarget: true, services: 'Postfix 3.6' }),
  devNode('redis-cache','zone-net-dmz', G13[2], { label: 'Redis Cache',  ip: '203.0.113.13', port: '6379', iconType: 'redis', color: '#ef4444', bg: 'rgba(239,68,68,0.08)',  isTarget: true, services: '⚠ NOAUTH' }),

  // ╔══════════════════════════════╗
  // ║  FW-VPN                      ║
  // ╚══════════════════════════════╝
  {
    id: 'fw-vpn', type: 'firewall',
    position: { x: 165, y: 880 },
    style: { width: 220, height: 160 },
    data: { label: 'FW-VPN', rules: ['VPN :443', 'Corp → Internal', 'Dev → Jenkins/Docker'] },
    draggable: true, selectable: false,
  },
  ...
  // ╚══════════════════════════════╝
  {
    ...
    style: { width: 220, height: 160 },
    data: { label: 'FW-01', rules: [':80/:443 → nginx', ':21 → FTP', ':25 → SMTP', '⚠ :6379 exposed'] },
    draggable: true, selectable: false,
  },
  ...
  devNode('admin-ep',      'zone-app', G22[0], { label: 'Admin Endpoint', ip: '10.0.1.1',  port: '8080', iconType: 'admin',    color: '#ff3366', bg: 'rgba(255,51,102,0.08)', isTarget: true, services: '/admin/create' }),
  devNode('ping-svc',      'zone-app', G22[1], { label: 'Ping Service',   ip: '10.0.1.2',  port: '9000', iconType: 'ping',     color: '#ff3366', bg: 'rgba(255,51,102,0.08)', isTarget: true, services: 'RCE ⚠ inject' }),
  devNode('exchange',      'zone-app', G22[2], { label: 'Exchange',       ip: '10.0.1.5',  port: '587',  iconType: 'exchange', color: '#00f0ff', bg: 'rgba(0,240,255,0.06)',  isTarget: true, services: 'Internal relay' }),
  devNode('redis-session', 'zone-app', G22[3], { label: 'Session Store',  ip: '10.0.1.10', port: '6380', iconType: 'session',  color: '#00f0ff', bg: 'rgba(0,240,255,0.06)',  isTarget: true, services: 'Redis · JWT' }),
  ...
  devNode('jenkins',    'zone-tools', G22[0], { label: 'Jenkins CI',  ip: '10.0.2.5', port: '8080', iconType: 'jenkins',    color: '#fbbf24', bg: 'rgba(251,191,36,0.07)', isTarget: true, services: 'Script console' }),
  devNode('confluence', 'zone-tools', G22[1], { label: 'Confluence',  ip: '10.0.2.6', port: '8090', iconType: 'confluence', color: '#3b82f6', bg: 'rgba(59,130,246,0.07)', isTarget: true, services: 'CVE-2022-26134' }),
  devNode('jira',       'zone-tools', G22[2], { label: 'Jira',        ip: '10.0.2.7', port: '8080', iconType: 'jira',       color: '#3b82f6', bg: 'rgba(59,130,246,0.07)',               services: 'Issue tracker' }),
  devNode('grafana',    'zone-tools', G22[3], { label: 'Grafana',     ip: '10.0.2.8', port: '3000', iconType: 'grafana',    color: '#f97316', bg: 'rgba(249,115,22,0.07)', isTarget: true, services: 'Default creds' }),

  // ╔══════════════════════════════╗
  // ║  FW-02 Internal              ║
  // ╚══════════════════════════════╝
  {
    id: 'fw-02', type: 'firewall',
    position: { x: 809, y: 880 },
    style: { width: 220, height: 160 },
    data: { label: 'FW-02', rules: ['BLOCK default', 'SSRF → Admin :8080', 'nginx → DB :1433'] },
    draggable: true, selectable: false,
  },
  ...
  // ╚══════════════════════════════╝
  {
    id: 'fw-03', type: 'firewall',
    position: { x: 735, y: 1490 },
    style: { width: 220, height: 160 },
    data: { label: 'FW-03 DB', rules: ['MSSQL :1433', 'PostgreSQL :5432', 'Redis :6380'] },
    draggable: true, selectable: false,
  },
  {
    id: 'fw-04', type: 'firewall',
    position: { x: 115, y: 1490 },
    style: { width: 220, height: 160 },
    data: { label: 'FW-04 Dev', rules: ['Docker :2376', 'K8s :6443', 'AWS :443'] },
    draggable: true, selectable: false,
  },
  ...
  devNode('mssql',    'zone-db', G13[0], { label: 'MSSQL',      ip: '10.0.3.5', port: '1433', iconType: 'mssql',    color: '#a855f7', bg: 'rgba(168,85,247,0.08)', services: 'SQL Server 2019' }),
  devNode('postgres', 'zone-db', G13[1], { label: 'PostgreSQL', ip: '10.0.3.6', port: '5432', iconType: 'postgres', color: '#a855f7', bg: 'rgba(168,85,247,0.08)', services: 'GraphQL DB' }),
  devNode('redis-db', 'zone-db', G13[2], { label: 'Redis DB',   ip: '10.0.3.7', port: '6379', iconType: 'redis',    color: '#a855f7', bg: 'rgba(168,85,247,0.08)', services: 'Persistent store' }),
  ...
  devNode('docker',     'zone-dev', G13[0], { label: 'Docker Host', ip: '10.0.4.5',  port: '2376', iconType: 'docker',     color: '#00ff88', bg: 'rgba(0,255,136,0.06)', services: 'Container runtime' }),
  devNode('kubernetes', 'zone-dev', G13[1], { label: 'Kubernetes',  ip: '10.0.4.6',  port: '6443', iconType: 'kubernetes', color: '#00ff88', bg: 'rgba(0,255,136,0.06)', services: 'k8s control' }),
  devNode('aws',        'zone-dev', G13[2], { label: 'AWS Gateway', ip: '10.0.4.10', port: '443',  iconType: 'aws',        color: '#00ff88', bg: 'rgba(0,255,136,0.06)', services: 'NAT · us-east-1' }),
];
...
// (dash and atk kept for future use)

const EDGES: Edge[] = [
  // Internet flow
  { id: 'e-cf-fw01',       source: 'cloudflare',     target: 'fw-01',          ...seg('#f97316') },
  { id: 'e-fw01-webdmz',   source: 'fw-01',           target: 'zone-web-dmz',   ...seg('#f59e0b') },
  { id: 'e-fw01-netdmz',   source: 'fw-01',           target: 'zone-net-dmz',   ...seg('#f59e0b') },

  // DMZ → Internal (SSRF attack path)
  { id: 'e-webdmz-fw02',   source: 'zone-web-dmz',   target: 'fw-02',          ...dash('#f59e0b') },
  { id: 'e-netdmz-fw02',   source: 'zone-net-dmz',   target: 'fw-02',          ...dash('#f59e0b') },
  { id: 'e-fw02-app',      source: 'fw-02',           target: 'zone-app',       ...atk() },

  // Endpoints → VPN → Internal Tools
  { id: 'e-endpoints-vpn', source: 'zone-endpoints', target: 'fw-vpn',         ...seg('#3b82f6') },
  { id: 'e-vpn-tools',     source: 'fw-vpn',          target: 'zone-tools',     ...dash('#3b82f6') },

  { id: 'e-vpn-app',     source: 'fw-vpn',          target: 'zone-app',     ...dash('#3b82f6') },

  // Internal → DBs
  { id: 'e-app-fw03',      source: 'zone-app',        target: 'fw-03',          ...seg('#00f0ff') },
  { id: 'e-fw03-db',       source: 'fw-03',           target: 'zone-db',        ...seg('#a855f7') },

  // Internal Tools → Dev
  { id: 'e-tools-fw04',    source: 'zone-tools',      target: 'fw-04',          ...seg('#00f0ff') },
  { id: 'e-fw04-dev',      source: 'fw-04',           target: 'zone-dev',       ...seg('#00ff88') },
